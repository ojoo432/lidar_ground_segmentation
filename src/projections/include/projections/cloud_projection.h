...
class CloudProjection {
 ...
 public:
  ...
  virtual void InitIntFromPoints(const std::vector<RichPoint>& points) = 0;
  ...
  const cv::Mat& intensity_image() const;
  const cv::Mat& row_angle_image() const;
  const cv::Mat& col_angle_image() const;
  cv::Mat& depth_image();
  cv::Mat& intensity_image();
  cv::Mat& row_angle_image();
  cv::Mat& col_angle_image();
  
  inline void CloneDepthImage(const cv::Mat& image) {
    _depth_image = image.clone();
  }
  inline void CloneIntensityImage(const cv::Mat& image) {
    _intensity_image = image.clone();
  }
 ...
 protected:
  ...
  cv::Mat _row_angle_image;
  cv::Mat _col_angle_image;
