...
ros::Publisher _pub_sample_points;
...
Mat CreateAngleImage(const Mat& depth_image, const ProjectionParams& _params) 
{
  ...
  for (int r = 1; r < angle_image.rows; ++r) 
  {
    ...
    for (int c = 0; c < angle_image.cols; ++c) 
    {
      ...
      // { 
      //   cout<<"---project---"<<endl;
      //   cout<<"x_down: "<<x_mat.at<float>(r, c)<<"x_top: "<<x_mat.at<float>(r - 1, c)<<endl;
      //   cout<<"y_down: "<<y_mat.at<float>(r, c)<<"y_top: "<<y_mat.at<float>(r - 1, c)<<endl;
      //   cout<<"dx: "<<dx<<"dy: "<<dy<<"angle: "<<atan2(dy, dx)<<endl;
      // }
...
Mat CreatePointImage(const Mat& depth_image, 
...
void sample_point_cloud(const Mat& depth_image,
...

{
  ...
  auto angle_image = CreatePointImage(depth_image, row_angle_image, col_angle_image);

  auto smoothed_image = ApplySavitskyGolaySmoothing(angle_image, _window_size);

  sample_point_cloud(depth_image, sample_cloud_ptr, row_angle_image, col_angle_image);
...
_ground_remove_angle, _window_size, params, seg_ground, row_angle_image, col_angle_image);
  ...
  publishCloud(&_pub_sample_points, sample_cloud_ptr);
...
int main(int argc, char **argv)
{
  ...
  _pub_sample_points = nh.advertise<sensor_msgs::PointCloud2>("/sample_points",1);
