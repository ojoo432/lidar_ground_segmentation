...
const  Radians _ground_remove_angle = 5_deg;
...

{
  ...
  params.SetSpan(SpanParams(-180_deg, 180_deg, 870),
                 SpanParams::Direction::HORIZONTAL);
  params.SetSpan(SpanParams(15_deg, -15_deg, 15),
                 SpanParams::Direction::VERTICAL);
  params.FillCosSin();
...
_ground_remove_angle, _window_size, params, seg_no_ground, row_angle_image);
  // cv::imwrite( "Smooth.jpg", smoothangleMap );

  auto ground_image = ZeroOutGroundBFS(depth_image, angle_image,
_ground_remove_angle, _window_size, params, seg_ground, row_angle_image);
  ...
  // cv::imshow("Image_Depth",depthMap);
